...
export function ItineraryPanel() {
  ...
  if (itemCount === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-line bg-surface p-6 text-center">
        <span
          aria-hidden
          className="mx-auto grid size-11 place-items-center rounded-xl bg-canvas text-muted"
        >
        ...
        <p className="mt-1 text-xs text-muted">
          Save a flight, stay, or activity and it will show up here as a
          day-by-day plan.
        </p>
      </div>
    );
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface">
      ...
      {issues.length > 0 ? (
        <ul
          className="space-y-1.5 border-b border-line bg-amber-50/60 px-4 py-3"
          aria-label="Trip warnings"
        >
                ...
                <li key={event.id} className="relative">
                  ...
                  <div className="group flex items-start gap-2 rounded-lg px-2 py-1.5 transition hover:bg-canvas">
                      ...
                      {event.type === "flight" ? (
                        <>
                          ...
                          <p className="text-[11px] text-muted">
                            ...
                            {event.leg.destinationCity ||
                              event.leg.destinationCode}{" "}
                            · {formatTime(event.leg.departure)}–
                            {formatTime(event.leg.arrival)}
                          </p>
                          {!event.isReturn ? (
                            <p className="text-[11px] text-muted">
                              {formatMoney(event.item.price)} ×{" "}
                              {pluralise(heads, "traveller")}
                            </p>
                          ) : null}
                        </>
                      ) : event.type === "hotel" ? (
                        <>
                          ...
                          <p className="text-[11px] text-muted">
                            {event.city}
                            {event.stay?.nights
                              ? ` · ${pluralise(event.stay.nights, "night")}`
                              : ""}
                          </p>
                          <p className="text-[11px] text-muted">
                            {event.item.perNightLabel ||
                              formatMoney(event.item.perNight)}{" "}
                            per night
                          </p>
                        </>
                      ) : (
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        toggleSaved(typeKey(event.type), event.item)
                      }
                      aria-label={`Remove ${
                        event.type === "flight"
                          ? event.leg.carrierName
      ...
      <dl className="space-y-1.5 border-t border-line px-4 py-3.5 text-xs">
        ...
        <div className="flex justify-between gap-3">
          <dt className="text-muted">
            {totals.bookedNights > 0
              ? `Stays · ${pluralise(totals.bookedNights, "night")}`
              : "Stays"}
          </dt>
          <dd className="font-semibold text-ink tabular-nums">
